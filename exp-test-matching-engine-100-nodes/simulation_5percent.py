...
class HashgridLab:
        ...
        async with self.semaphore:
            try:
                ...
                for msg in messages:
                    ...
                    dot_product = np.dot(peer_is, data['seek'])
                    score = float(sigmoid(dot_product))
                    scores.append(score)
    ...
    async def run(self):
        ...
        async for tick in grid.listen(poll_interval=0.5):
            ...
            # --- EXPERIMENT: 5% șansă de mutație agresivă (0.5) ---
            for nid in self.nodes_registry:
                if random.random() < 0.05:
                    old_seek = self.nodes_registry[nid]['seek']
                    noise = np.random.uniform(-0.5, 0.5, VECTOR_SIZE)
                    self.nodes_registry[nid]['seek'] = np.clip(old_seek + noise, -1, 1)
            # ------------------------------------------------------
